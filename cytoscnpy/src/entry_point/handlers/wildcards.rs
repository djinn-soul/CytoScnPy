//! Handler for the `wildcards` star-import detection subcommand.
...
use crate::wildcards::{analyze_wildcards, print_json_report, print_terminal_report};

/// CLI flags specific to the `wildcards` command.
#[derive(Debug, Clone, Copy)]
pub(crate) struct WildcardsFlags {
    /// Whether to output JSON format.
    pub json: bool,
    /// Whether to exit with code 1 when any wildcard import is found.
    pub fail_on_any: bool,
    /// Optional upper limit for wildcard import count.
    pub max_wildcards: Option<usize>,
    /// Verbose output mode.
    pub verbose: bool,
}

/// Executes the `wildcards` subcommand.
pub(crate) fn handle_wildcards<W: Write>(
    paths: &PathArgs,
    flags: WildcardsFlags,
    ...
    let result = analyze_wildcards(&targets, &excludes, flags.verbose);
    ...
    if has_failure && !flags.json {
        writeln!(
            writer,
            "\n[GATE] {} wildcard import(s) detected - FAILED",
            result.stats.total
        )?;
    }
...
fn determine_failure(result: &crate::wildcards::WildcardsResult, flags: &WildcardsFlags) -> bool {
    if flags.fail_on_any && !result.is_clean() {
        return true;
    }
    if let Some(limit) = flags.max_wildcards {
