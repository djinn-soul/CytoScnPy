...
pub fn print_sarif(writer: &mut impl Write, result: &AnalysisResult) -> std::io::Result<()> {
    ...
    let mut add = |id: &str, msg: &str, file: &str, line: usize, severity: &str| {
        ...
        results.push(SarifResult {
            ...
            message: Message { text: msg.to_string() },
            locations: vec![Location {
                physical_location: PhysicalLocation {
    ...
    for f in &result.danger {
        add(&f.rule_id, &f.message, &f.file.to_string_lossy(), f.line, &f.severity);
    }
    // Secrets
    for s in &result.secrets {
        add(&s.rule_id, &s.message, &s.file.to_string_lossy(), s.line, "HIGH");
    }
    // Quality
    for q in &result.quality {
        add(&q.rule_id, &q.message, &q.file.to_string_lossy(), q.line, &q.severity);
    }
    // Taint
    for t in &result.taint_findings {
        let msg = format!("{} (Source: {})", t.vuln_type, t.source);
        add(&t.rule_id, &msg, &t.file.to_string_lossy(), t.sink_line, &t.severity.to_string());
    }
    // Unused
    for f in &result.unused_functions {
        add("unused-function", &format!("Unused function: {}", f.name), &f.file.to_string_lossy(), f.line, "LOW");
    }
    for c in &result.unused_classes {
        add("unused-class", &format!("Unused class: {}", c.name), &c.file.to_string_lossy(), c.line, "LOW");
    }
    for i in &result.unused_imports {
        add("unused-import", &format!("Unused import: {}", i.name), &i.file.to_string_lossy(), i.line, "LOW");
    }
    for v in &result.unused_variables {
        add("unused-variable", &format!("Unused variable: {}", v.name), &v.file.to_string_lossy(), v.line, "LOW");
    }
    for m in &result.unused_methods {
        add("unused-method", &format!("Unused method: {}", m.name), &m.file.to_string_lossy(), m.line, "LOW");
    }
    for p in &result.unused_parameters {
        add("unused-parameter", &format!("Unused parameter: {}", p.name), &p.file.to_string_lossy(), p.line, "LOW");
    }

    // Parse Errors
    for e in &result.parse_errors {
        add("parse-error", &format!("Parse error: {}", e.error), &e.file.to_string_lossy(), 0, "HIGH");
    }
