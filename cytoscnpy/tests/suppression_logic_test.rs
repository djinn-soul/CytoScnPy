...
fn test_suppression_logic_scenarios() {
    ...
    // Note: get_ignored_lines now returns a map of blocked lines.
    // Ideally this shouldn't contain the key if it's ignored for OTHER tools but not CSP.
    // However, if get_ignored_lines returns specific suppressions, checking existence isn't enough.
    // We should check if it suppresses CSP.
    // But for this quick fix I will assume the previous tests implied .contains() meant "is in the map".
    // Wait, test 7 asserts !contains.
