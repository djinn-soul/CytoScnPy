...
suite('CytoScnPy Extension Test Suite', function() {
	...
	before(async function() {
		...
		const doc = await vscode.workspace.openTextDocument({ language: 'python', content: 'print("hello world")' });
	...
	test('Extension should be active', async () => {
		const extension = vscode.extensions.getExtension('cytoscnpy.cytoscnpy');
